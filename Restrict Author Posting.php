<?php
/*
Plugin Name: Restrict Author Posting by Custom Taxonomy
Plugin URI: Propietary
Description: Allow you to restrict the taxonomy that your site contributors can post to
Author: Nzureuz, Mijo, (original-source)Mcjambi
Version: 1.0
Tags:	restrict user, banned user, user role, posting to category, specific posting category, author role
Author URI: Propietary
*/

/*
Copyright 2015 Nzureuz, Mijo, (original-source)Mcjambi
...
*/
		...
		function restrict_user_form( $user ) {
			...
			$args = array(
				...
				'depth'              => 0,
				'tab_index'          => 0,
				'taxonomy'           => 'agencia', //Here goes the name of the taxonomy.
				'hide_if_empty'      => false,
				'walker'             => ''
			);
		?>
		...
		function save_restrict_post( $post_id ) {
			...
			remove_action('save_post', 'save_restrict_post');
				wp_set_object_terms( $post_id, (int)get_user_meta( get_current_user_id() , '_access', true), 'agencia' );
			add_action('save_post', 'save_restrict_post');
			}
		}
		...
		/*Change # for the id of the taxonomy to hide*/
		function restrict_remove_meta_boxes() {
			if (is_restrict() )
				remove_meta_box('{agencia}div', 'post', 'normal');
		}
